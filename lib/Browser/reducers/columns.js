...
function createColumn(position=0,path='',type,handlerName){
	...
	,	position
	,	status:STATUS_NONE
	,	directory:type==TYPE_DIRECTORY?path:false
	,	file:type==TYPE_FILE?path:false
	,	info:false
	,	type
	,	handlerName
	,	selectedItem:0
...
export function columns(state=defaultState,action){
	switch(action.type){
		...
		case REQUEST_FILE_CONTENTS:
			return state.map(column =>
				column.id == action.columnId ?
					Object.assign({},column,{status:STATUS_LOADING,file:action.file}) :
					column
			)
		...
		case RECEIVE_FILE:
			return state.map(column =>
				(column.id == action.columnId) && (column.file == action.file)?
		...
		case RECEIVE_FILE_CONTENTS:
			return state.map(column =>
				(column.id == action.columnId) && (column.file == action.file)?
					Object.assign({},column,{status:STATUS_LOADED,contents:action.contents}) :
					column
			)
		case ERROR_FETCHING:
			return state.map(column=>
